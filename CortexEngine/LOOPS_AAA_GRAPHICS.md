...
- Loop 30 contract:
  - Invariant: generated exteriors must have scene-wide lighting/material response evidence rather than flat global light plus per-object overlays: bounded shadowed local lights, a key/fill ratio, ambient clamp, renderer SSAO/shadow-map targets, local probe volumes, contact shadow bands, and existing-surface PBR tuning.
  - Entry: Loop 29 checkpoint is green, but visual inspection shows the scene still reads flat and shadow-light response is not shaping the full frame.
  - Scope in: `tools/scene_compiler.py`, `tools/scene_graphics_gate.py`, `src/Core/Engine_Scenes.cpp`, campaign ledgers, novel generated exterior artifacts.
  - Scope out: weakening `tools/scene_quality_gate.py`, forcing generated DXR by default, exceeding three shadowed local spotlights, visible light cards, unrelated interiors, or prompt-specific hacks.
  - Verifier: strengthen `scene_graphics_gate.py` so Loop 29 campsite/desert/alpine fail `missing_lighting_shadow_material_field`; then require IR/runtime evidence for shadowed lights, material response surfaces, contact shadow bands, local probes, key/fill, ambient clamp, SSAO intensity, shadow bias, and shadow PCF.
  - Exit: verifier green plus visual inspection shows stronger light/shadow/material response without obvious painted bands or black-mass regressions; final AAA call remains `HUMAN-GATE`.
  - Escape: stop if the pass becomes a painted decal trick, crushes background/source assets, breaks water/color ROIs, or destabilizes the shadow budget.
- Loop 30 red proof:
  - Loop 29 final campsite/desert/alpine artifacts failed the strengthened graphics gate with `missing_lighting_shadow_material_field`.
  - The first attempt to use `--expect-fail` was the wrong oracle mode for this red proof because it expects the older known-bad fixture failure codes; rerun without that flag confirmed the single new failure on all three artifacts.
- Loop 30 verifier evidence:
  - Implementation: `tools/scene_compiler.py` emits `graphics_pass.lighting_shadow_material_field` and material zones for shadowed key response, ambient-clamped contact shadows, local probes, raking shadow bands, and scene-wide PBR response. `tools/scene_graphics_gate.py` requires the IR/runtime contract. `src/Core/Engine_Scenes.cpp` parses the contract, tunes existing generated renderables, adds local reflection probe volumes, creates bounded terrain-toned contact shadow bands, creates two or three shadow-casting spotlights within the engine budget, and applies renderer ambient/SSAO/shadow bias/PCF/tone controls.
  - Visual refinement before checkpoint: first render `aaa_light_field_*_loop30` made foreground shadow bands read as painted marks and crushed background masses. The final `loop30b` pass reduced band size/contrast, skipped ridge/backdrop material crushing, and raised applied ambient within the contract.
  - Python compile exited 0.
  - Release build exited 0: `[OK] Build complete in 48.6s`; final no-work rebuild exited 0: `[OK] Build complete in 3.5s`.
  - `git diff --check` exited 0 except expected CRLF warnings.
  - Campsite `aaa_light_field_campsite_loop30b` rendered VALID; quality green (`purple_fraction=0.7940`, `avg_luma=0.3088`, `nonblack_fraction=0.9729`); graphics green (`dark_contact_area_fraction=0.4291`); Director validation green; runtime `shadowed_lights=3 material_surfaces=34 shadow_bands=20 probe_volumes=2 key_fill=3.35 ambient=0.55 ssao=3.35 shadow_bias=0.0011 shadow_pcf=3.85`.
  - Desert `aaa_light_field_desert_loop30b` rendered VALID; quality green (`turquoise_fraction=0.3957`, `avg_luma=0.2354`, `nonblack_fraction=0.8820`); graphics green (`dark_contact_area_fraction=0.5644`); Director validation green; runtime `shadowed_lights=3 material_surfaces=30 shadow_bands=20 probe_volumes=2 key_fill=3.35 ambient=0.55 ssao=3.35 shadow_bias=0.0011 shadow_pcf=3.85`.
  - Alpine `aaa_light_field_alpine_loop30b` rendered VALID; quality green (`avg_luma=0.1051`, `cool_fraction=0.8853`, `nonblack_fraction=0.7582`); graphics green (`dark_contact_area_fraction=0.8534`); Director validation green; runtime `shadowed_lights=2 material_surfaces=34 shadow_bands=16 probe_volumes=2 key_fill=2.65 ambient=0.50 ssao=3.05 shadow_bias=0.0013 shadow_pcf=3.55`.
  - Kitchen smoke `regression_kitchen_aaa_loop30` rendered VALID and quality green (`avg_luma=0.4776`, `nonblack_fraction=1.0`).
  - Known-bad quality oracle `gen_a_foggy_mountain_campsite_beside_0` with `--expect-fail` still reports forbidden fridge assets, missing ridge, focal visibility, and purple-water failures. Known-bad graphics oracle `v3_campsite_ridge_test_0` with `--expect-fail` still fails and now includes `missing_lighting_shadow_material_field`.
  - Visual inspection: Loop 30 is a renderer/material response checkpoint, not final AAA. Lighting depth and contact evidence are stronger and the worst painted-band artifact is reduced, but the pass exposes crushed black source/backdrop masses, especially desert and alpine. The next loop should add a crushed-black/source-readability guard and rebalance source/backdrop material response.
- Loop 31 contract:
  - Invariant: generated exteriors must avoid crushed black source/backdrop masses after the lighting/shadow field pass while preserving prompt color ROIs, contrast, and renderer shadow/occlusion evidence.
  - Entry: Loop 30b campsite/desert/alpine are green but visual inspection shows black source/backdrop masses and lower nonblack fractions, especially desert/alpine.
  - Scope in: `tools/scene_compiler.py`, `tools/scene_graphics_gate.py`, `src/Core/Engine_Scenes.cpp`, campaign ledgers, novel generated exterior artifacts.
  - Scope out: weakening `tools/scene_quality_gate.py`, forcing generated DXR by default, global washout/exposure-only fixes, visible proof markers/cards, unrelated interiors, or prompt-specific hacks.
  - Verifier: strengthen `scene_graphics_gate.py` so Loop 30b campsite/desert/alpine fail `missing_source_readability_balance`; then require IR/runtime source/backdrop readability evidence plus frame nonblack/luma floors on fresh campsite/desert/alpine renders.
  - Exit: verifier green, kitchen and known-bad oracles green/expected-fail, Release build green, and visual inspection confirms the first marker artifact is removed. Final AAA call remains `HUMAN-GATE`.
  - Escape: stop if the pass washes out water/color ROIs, creates visible bars/cards, or merely moves the crushed-black artifact into another obvious overlay.
- Loop 31 red proof:
  - Loop 30b campsite, desert, and alpine artifacts fail the strengthened graphics gate with `missing_source_readability_balance`; campsite shows `frame_nonblack_fraction=0.9554`, desert `0.8187`, alpine `0.6339` under the new frame metric.
- Loop 31 verifier evidence:
  - Heartbeat proof `aaa-graphics-proof` fired by timeout after 2s.
  - Implementation: `tools/scene_compiler.py` emits `graphics_pass.source_readability_balance`; `tools/scene_graphics_gate.py` computes frame-level luma/nonblack metrics and requires IR/runtime readability evidence; `src/Core/Engine_Scenes.cpp` parses the contract, lifts source/backdrop albedo floors, reduces source/backdrop occlusion strength, and applies bounded ambient/exposure/IBL/SSAO rebalance.
  - Visual refinement before checkpoint: first Loop 31 renders improved black-mass readability but created visible upright split-marker rows along the campsite/desert shoreline. Final `loop31b` lowered the split count, changed split proof geometry from upright cubes into terrain-following blended ribbons, and corrected the occlusion rebalance from `max()` to `min()` so the pass reduces over-occlusion.
  - Python compile exited 0.
  - Release build exited 0: `[OK] Build complete in 50.6s`; final no-work rebuild exited 0: `[OK] Build complete in 3.0s`.
  - `git diff --check` exited 0 except expected CRLF warnings.
  - Campsite `aaa_source_read_campsite_loop31b` rendered VALID; quality green (`purple_fraction=0.7416`, `avg_luma=0.3446`, `nonblack_fraction=0.9901`); graphics green (`frame_avg_luma=0.3459`, `frame_nonblack_fraction=0.9831`, `dark_contact_area_fraction=0.1275`); Director validation green; runtime `source_surfaces=34 backdrop_surfaces=14 black_splits=10 albedo_floor=0.12 nonblack_floor=0.92`.
  - Desert `aaa_source_read_desert_loop31b` rendered VALID; quality green (`turquoise_fraction=0.3803`, `avg_luma=0.2904`, `nonblack_fraction=0.9846`); graphics green (`frame_avg_luma=0.2916`, `frame_nonblack_fraction=0.9620`, `dark_contact_area_fraction=0.1721`); Director validation green; runtime `source_surfaces=34 backdrop_surfaces=12 black_splits=10 albedo_floor=0.12 nonblack_floor=0.92`.
  - Alpine `aaa_source_read_alpine_loop31b` rendered VALID; quality green (`avg_luma=0.1551`, `cool_fraction=0.8843`, `nonblack_fraction=0.9821`); graphics green (`frame_avg_luma=0.1553`, `frame_nonblack_fraction=0.9398`, `dark_contact_area_fraction=0.6178`); Director validation green; runtime `source_surfaces=28 backdrop_surfaces=14 black_splits=8 albedo_floor=0.10 nonblack_floor=0.82`.
  - Kitchen smoke `regression_kitchen_aaa_loop31` rendered VALID and quality green (`avg_luma=0.4462`, `nonblack_fraction=1.0`).
  - Known-bad quality oracle `gen_a_foggy_mountain_campsite_beside_0` with `--expect-fail` still reports forbidden fridge assets, missing ridge, focal visibility, and purple-water failures. Known-bad graphics oracle `v3_campsite_ridge_test_0` with `--expect-fail` still fails and now includes `missing_source_readability_balance`.
  - Visual inspection: the upright marker artifact is gone in `loop31b`, and source/backdrop black masses are less severe without washing out prompt water colors. This is still not AAA: assets remain stylized, many props are kit-like, and deeper material/shader/occlusion architecture is still the next serious front.
- Loop 32 contract:
  - Invariant: every generated exterior artifact that passes the graphics gate must carry per-render frame-contract evidence that the actual renderer executed the visibility-buffer material/deferred/full-scene shader path for that PNG, not just generator metadata or initialization logs.
  - Entry: Loop 31 checkpoint is green, but visual inspection remains far from AAA and the verifier cannot attribute actual `VBMaterialResolve`/`VBDeferredLighting` execution to each generated still. `frame_report_last.json` contains the needed ground truth, but generated artifacts do not snapshot it.
  - Scope in: `tools/scene_gen.py`, `tools/scene_graphics_gate.py`, campaign ledgers, generated exterior artifacts.
  - Scope out: weakening `tools/scene_quality_gate.py`, forcing generated DXR by default, accepting stale global `frame_report_last.json` as per-artifact evidence, unrelated renderer rewrites, unrelated interiors, or prompt-specific hacks.
  - Verifier: strengthen `scene_graphics_gate.py` so Loop 31b campsite/desert/alpine fail `missing_full_scene_shader_pipeline_evidence` without a sidecar frame report; then require a fresh `<out_name>_frame_report.json` proving visibility buffer enabled/rendered, `VBMaterialResolve`, `VBDeferredLighting`, valid `visibility_buffer`/`vb_gbuffer_*` resources, positive VB draw/material counts, and FullSceneShaderPipelineV3 `material_attributes_ready` plus `lighting_adapter_ready`.
  - Exit: verifier green on fresh campsite/desert/alpine renders using inferred sidecars, quality/Director gates green, known-bad oracles still fail for the right reasons, Python compile and Release build green, and visual inspection is recorded honestly. Final AAA call remains `HUMAN-GATE`.
  - Escape: stop if frame reports are stale/unattributable, if generated exteriors are not actually executing the VB/deferred path, or if the gate can pass without per-artifact frame-contract evidence.
- Loop 32 red/recon proof:
  - Heartbeat proof `aaa-loop32-proof` fired by timeout after 1s.
  - Recon: `build/bin/logs/frame_report_last.json` has `features.visibility_buffer_enabled=true`, `culling.visibility_buffer_rendered=true`, positive VB instance/material/draw counts, executed pass records for `VBMaterialResolve` and `VBDeferredLighting`, valid `vb_gbuffer_*` resources, and FullSceneShaderPipelineV3 top-level `material_attributes_ready=true` and `lighting_adapter_ready=true`.
  - Current gap: old Loop 31b artifacts have no `<out_name>_frame_report.json` sidecar, so the new verifier must fail them before implementation.
- Loop 32 verifier evidence:
  - Implementation: `tools/scene_gen.py` copies a fresh `frame_report_shutdown.json`/`frame_report_last.json` to `build/bin/logs/<out_name>_frame_report.json` only when it was produced after the current render started. `tools/scene_graphics_gate.py` infers that sidecar from `--png`, `--ir`, or `--log`, or accepts `--frame-report`, and requires per-render evidence for visibility buffer enabled/rendered, `VBMaterialResolve`, `VBDeferredLighting`, valid `visibility_buffer`/`vb_gbuffer_*` resources, positive VB draw/material counts, and FullSceneShaderPipelineV3 material/lighting adapter readiness.
  - Red proof: old `aaa_source_read_campsite_loop31b_0`, `aaa_source_read_desert_loop31b_0`, and `aaa_source_read_alpine_loop31b_0` all fail the strengthened graphics gate with only `missing_full_scene_shader_pipeline_evidence` and `frame_report_sidecar`.
  - Python compile exited 0.
  - Release rebuild exited 0: `[OK] Build complete in 3.4s`.
  - `git diff --check` exited 0 except expected CRLF warnings.
  - Campsite `aaa_pipeline_campsite_loop32` rendered VALID; quality green (`purple_fraction=0.7414`, `avg_luma=0.3425`, `nonblack_fraction=0.9899`); graphics green (`frame_nonblack_fraction=0.9822`, `dark_contact_area_fraction=0.1376`); Director validation green; frame pipeline `VB instances=594 materials=463 batches=74 gpu_ms=7.041`, `VBMaterialResolve=true`, `VBDeferredLighting=true`, `material_attributes_ready=true`, `lighting_adapter_ready=true`.
  - Desert `aaa_pipeline_desert_loop32` rendered VALID; quality green (`turquoise_fraction=0.3786`, `avg_luma=0.2889`, `nonblack_fraction=0.9843`); graphics green (`frame_nonblack_fraction=0.9602`, `dark_contact_area_fraction=0.1886`); Director validation green; frame pipeline `VB instances=611 materials=469 batches=70 gpu_ms=17.5688`, `VBMaterialResolve=true`, `VBDeferredLighting=true`, `material_attributes_ready=true`, `lighting_adapter_ready=true`.
  - Alpine `aaa_pipeline_alpine_loop32` rendered VALID; quality green (`avg_luma=0.1535`, `cool_fraction=0.8844`, `nonblack_fraction=0.9790`); graphics green (`frame_nonblack_fraction=0.9372`, `dark_contact_area_fraction=0.6289`); Director validation green; frame pipeline `VB instances=532 materials=460 batches=65 gpu_ms=11.6091`, `VBMaterialResolve=true`, `VBDeferredLighting=true`, `material_attributes_ready=true`, `lighting_adapter_ready=true`.
  - Kitchen smoke `regression_kitchen_aaa_loop32` rendered VALID and quality green (`avg_luma=0.2957`, `nonblack_fraction=1.0`).
  - Known-bad quality oracle `gen_a_foggy_mountain_campsite_beside_0` with `--expect-fail` still reports forbidden fridge assets, missing ridge, focal visibility, and purple-water failures. Known-bad graphics oracle `v3_campsite_ridge_test_0` with `--expect-fail` still fails and now also reports `missing_full_scene_shader_pipeline_evidence`.
  - Visual inspection: the sidecar evidence is real, but this is not a visual AAA win by itself. Campsite has clear purple water and camp/fire read but remains cluttered with low-poly mountains/trees and hard flat lake banding. Desert has readable turquoise river/canyon flanks but still depends on slab side walls and a flat horizon. Alpine has a stronger moonlit cabin read but still shows low-poly trees, flat lake/backdrop layering, and scattered debris. The next loop must attack high-fidelity terrain/backdrop/water and scene cleanup, not another provenance-only pass.
