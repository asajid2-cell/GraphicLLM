...
cbuffer FrameConstants : register(b0, space0)
{
    ...
    float4   g_FogParams;
    float4   g_AOParams;
    // Bloom params are present in the C++ FrameConstants (ShaderTypes.h) but
    // not used directly in this shader; include them to keep the layout
    // byte-for-byte compatible so subsequent fields (TAA, matrices) line up.
...
void RayGen_Shadow()
{
    ...
    // Use light 0 as the sun (directional).
    float3 sunDir = float3(0.0f, -1.0f, 0.0f);
    if (g_LightCount.x > 0)
    {
        sunDir = normalize(g_Lights[0].direction_cosInner.xyz);
        // g_Lights stores direction from surface to light; invert for ray dir.
        sunDir = -sunDir;
    }
