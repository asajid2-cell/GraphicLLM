// Ground-truth ambient occlusion fullscreen pass.
...
float4 ComputeGTAO(float2 uv, float2 pixel)
{
    ...
    float2 texel = max(g_PostParams.xy * 2.0f, float2(1e-6f, 1e-6f));
...
{
    return ComputeGTAO(input.uv, input.position.xy);
}
