// Screen-space ambient occlusion compute shader (async compute version).
// Replaces the raster-based SSAO pass for use on the async compute queue.
...
{
    // Invert the UV mapping to get clip space.
...
void CSMain(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    float2 uv = (float2(dispatchThreadId.xy) + 0.5f) / float2(outputDim);

    // If SSAO is disabled, output no occlusion (1.0).
    if (g_AOParams.x <= 0.5f)
    {
        g_OutputAO[dispatchThreadId.xy] = float4(1.0f, 1.0f, 1.0f, 1.0f);
        return;
    }
    ...
    g_OutputAO[dispatchThreadId.xy] = float4(ao, ao, ao, 1.0f);
}
