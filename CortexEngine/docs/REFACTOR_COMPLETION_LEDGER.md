...
logs=CortexEngine/build/bin/logs/runs/release_validation_20260510_231517_257_5328_096ca2b9
```
...
- Temporal validation: `gpu_ms=1.391`, `disocclusion=0.006671`,
  `high_motion=0.005177`, `object_motion=0.0731`, `visible=7`, `warnings=0`.
- Temporal camera cut: `frames=53`, `cut_frame=20`,
  `camera=reflection_closeup`, `gpu_ms=3.087`,
  `rt_reflection_reset=camera_cut`, `invalidated_frame=20`.
- RT showcase: `frames=33`, `gpu_ms=2.297/16.7`,
...
| P3-GLOBAL-03 | Reviewer can switch environments and IBLs safely. | PARTIAL | SRC-ENV-P3, SRC-UI-P3 | `tools/run_ibl_gallery_tests.ps1 -NoBuild`; `tools/run_environment_manifest_tests.ps1` | IBL gallery passed all five enabled runtime IBLs: `studio`, `warm_gallery`, `sunset_courtyard`, `cool_overcast`, `night_city`. Manifest passed. | Interactive environment switching and missing-physical-asset fallback are not fully matrixed. |
...
| P3-WS-04 | Backgrounds, skies, and IBL library. | PARTIAL | SRC-ENV-P3 | `tools/run_environment_manifest_tests.ps1`; `tools/run_ibl_gallery_tests.ps1 -NoBuild` | Manifest passed; uncapped IBL gallery passed all five enabled runtime IBLs with nonblack visual captures. | Procedural fallback runtime matrix and asset tooling incomplete. |
...
| P3-3D | Phase 3 Visual Matrix Script. | DONE_VERIFIED | `tools/run_phase3_visual_matrix.ps1` | `tools/run_phase3_visual_matrix.ps1 -NoBuild -TemporalSmokeFrames 90 -RTSmokeFrames 180 -SkipSurfaceDebug` | Latest uncapped matrix passed all rows and ran the IBL gallery for all enabled runtime IBLs. Logs: `phase3_visual_matrix_20260510_231344_693_26152_8a1c343f`. | Matrix is narrower than original blueprint cases: no explicit safe_startup/fallback_sky row. |
...
| P3-3K | Environment Library Runtime. | PARTIAL | `Renderer_Environment.cpp`, `RendererEnvironmentState.h`, SRC-ENV-P3 | `tools/run_ibl_gallery_tests.ps1 -NoBuild`; release gate | IBL gallery passed for all enabled runtime IBLs: `studio`, `warm_gallery`, `sunset_courtyard`, `cool_overcast`, `night_city`. | Missing physical-asset runtime fallback is not fully tested in latest gate. |
