...
| SRC-ENV-P3 | `src/Graphics/EnvironmentManifest.cpp::LoadEnvironmentManifest`, `IsEnvironmentAllowedForBudget`, `src/Graphics/Renderer_Environment.cpp`, `src/Graphics/RendererEnvironmentState.h`, `src/Core/StartupPreflight.cpp`, `assets/environments/environments.json` |
...
| P2-SNAPSHOT-03 | Bloom transient validation passes in alias/no-alias modes and bloom transients are default-on. | PARTIAL | SRC-RENDERGRAPH, `Renderer_RenderGraphBloom.cpp::Renderer::ExecuteBloomInRenderGraph`, `Renderer_RenderGraphEndFrame.cpp::Renderer::ExecuteEndFrameInRenderGraph` | Historical: `CORTEX_RG_TRANSIENT_VALIDATE=1 tools/run_rt_showcase_smoke.ps1 -NoBuild -IsolatedLogs`; current: `tools/run_release_validation.ps1` | Current RT showcase logs show `Bloom RG: fused graph-owned intermediates enabled (default)` and `transient_delta=0`. | Latest release gate does not explicitly run both alias/no-alias transient modes. Add focused transient matrix to current release gate before marking fully done. |
...
| P2-08AM | Pass 8AM - Render Graph Validation Module | DONE_UNVERIFIED | `Renderer_RenderGraphDiagnostics.cpp`, SRC-RENDERGRAPH | historical + release smoke | Current release gate does not explicitly run every RG validation env mode. |
...
| P2-10B | Pass 10B - Debug Primitive System Boundaries | DONE_UNVERIFIED | `RendererDebugLineState.h`, `Renderer_DebugLines.cpp` | no focused current test found | Debug lines are not a release smoke focus. |
...
| P2-10D | Pass 10D - Source List Stub Cleanup | DONE_UNVERIFIED | build/source list files | build step in release gate | Build passes; no specific source-list audit was rerun. |
...
| P2-11CY | Pass 11CY - Public Repository Hygiene | DONE_UNVERIFIED | `.gitignore`, removed artifacts | historical `phase2.md` evidence | No current hygiene command was run in this audit except worktree status. | Run `git diff --check` and ignored-file audit before a public release. |
...
| REM-05 | Full render-graph transient alias validation matrix. | PARTIAL | Current release checks descriptor deltas and bloom default path, but not every alias/no-alias mode from older Phase 2 notes. |
...
| P3-GLOBAL-02 | Reviewer can launch without setup surprises. | PARTIAL | SRC-PREFLIGHT, SRC-DOCS | `tools/run_release_validation.ps1`; `tools/run_fatal_error_contract_tests.ps1` | Startup preflight logs `passed=true`; fatal contract passed. | Positive install/setup path and missing optional asset matrix are not fully covered. |
| P3-GLOBAL-03 | Reviewer can switch environments and IBLs safely. | PARTIAL | SRC-ENV-P3, SRC-UI-P3 | `tools/run_ibl_gallery_tests.ps1 -NoBuild`; `tools/run_environment_manifest_tests.ps1` | IBL gallery passed for three environments; manifest passed. | Interactive environment switching and missing-selected fallback are not fully matrixed. |
...
| ID | Pass | Status | Source/functions | Validation command | Latest evidence | Remaining work |
|---|---|---|---|---|---|---|
| P3-3A | Startup Preflight Contract. | PARTIAL | SRC-PREFLIGHT, SRC-CONTRACT | `tools/run_release_validation.ps1` | Latest runtime logs show `Startup preflight: passed=true safe_mode=false profile=default warnings=0 errors=0`. | Missing optional manifest, missing IBL, RT-disabled, and safe-profile preflight variants are not all current release-gate cases. |
...
| P3-3I5 | IBL Asset Policy Lock. | PARTIAL | `assets/environments/environments.json`, SRC-ENV-P3 | `tools/run_environment_manifest_tests.ps1` | Manifest policy passes and default/fallback are declared. | Exact optional download/generation tooling and missing selected asset runtime variants incomplete. |
...
| P3-3K | Environment Library Runtime. | PARTIAL | `Renderer_Environment.cpp`, `RendererEnvironmentState.h`, SRC-ENV-P3 | `tools/run_ibl_gallery_tests.ps1 -NoBuild`; release gate | IBL gallery passed for `studio`, `warm_gallery`, `sunset_courtyard`. | All enabled IBLs and missing-asset runtime fallback are not fully tested in latest gate. |
| P3-3L | Procedural Sky and Fallback Background. | PARTIAL | `assets/shaders/ProceduralSky.hlsl`, SRC-ENV-P3 | `tools/run_environment_manifest_tests.ps1`; startup logs | Fallback is declared and procedural sky pipeline creates. | Runtime with environment assets unavailable was not run. |
| P3-3M | IBL Review Scene / IBL Gallery. | DONE_VERIFIED | `tools/run_ibl_gallery_tests.ps1`, SRC-SCENES, SRC-ENV-P3 | `tools/run_ibl_gallery_tests.ps1 -NoBuild -MaxEnvironments 3` | IBL gallery passed three environments in latest release gate. | Original "all enabled IBLs" only partially covered by max=3 in release gate. |
...
|---|---|---|---|---|---|---|
| P3-FC-STARTUP | `startup` section. | PARTIAL | SRC-PREFLIGHT, SRC-CONTRACT | `tools/run_release_validation.ps1`; `tools/run_phase3_fallback_matrix.ps1 -NoBuild` | Startup preflight logs pass; fallback matrix passed safe-mode/no-RT cases. | Missing optional manifest/asset variants are still not fully covered. |
...
| P3-GATE-07 | IBL gallery matrix. | PARTIAL | `tools/run_ibl_gallery_tests.ps1` | `tools/run_ibl_gallery_tests.ps1 -NoBuild -MaxEnvironments 3` | Three envs passed. | Original asks all enabled IBLs; latest release caps at 3. |
...
| P3-READY-03 | Environment/IBL policy and manifest are not implemented. | PARTIAL | SRC-ENV-P3 | `tools/run_environment_manifest_tests.ps1`; `tools/run_ibl_gallery_tests.ps1`; `tools/run_phase3_fallback_matrix.ps1 -NoBuild` | Manifest, IBL runtime, and missing selected environment fallback reporting pass. | Asset policy tooling and missing physical asset matrix incomplete. |
...
| P3-ORDER-05 | Add startup preflight contract for assets/config/environment. | PARTIAL | SRC-PREFLIGHT | release gate; `tools/run_phase3_fallback_matrix.ps1 -NoBuild` | Default startup preflight and safe-mode fallback matrix pass. | Missing optional manifest/asset variants incomplete. |
| P3-ORDER-06 | Lock IBL asset policy. | PARTIAL | `environments.json`, SRC-ENV-P3 | environment manifest test | Manifest passes. | Tooling/download/generation rules incomplete. |
| P3-ORDER-07 | Add environment manifest parser and procedural fallback. | PARTIAL | `EnvironmentManifest.cpp`, procedural sky shader, environment fallback contract fields | manifest/IBL/fallback tests | Parser, fallback declaration, and missing selected environment fallback reporting exist and pass. | Runtime no-assets physical fallback test missing. |
...
| P3-DOD-03 | Robust environment/IBL library with fallback behavior. | PARTIAL | SRC-ENV-P3 | manifest/IBL/fallback tests | Manifest/gallery pass; missing selected environment fallback matrix passes. | Missing physical asset/no-assets fallback matrix incomplete. |
...
| P3-DOD-15 | No known descriptor, memory, startup, screenshot regression hidden by test suite. | PARTIAL | tools scripts, SRC-BUDGET, SRC-VISUAL | release gate plus new focused gates | Current suite catches many regressions; fallback, screenshot negative, particle-disabled, and RT outlier focused gates now exist and pass individually. | Descriptor stress, full transient matrix, full ownership audit, camera-cut validation, visual probe/golden comparison, and public packaging remain incomplete. |
