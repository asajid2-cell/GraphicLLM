...
| P3-3S | RT Reflection Tuning Controls. | PARTIAL | SRC-RT, SRC-UI-P3 | `tools/run_rt_showcase_smoke.ps1 -NoBuild -IsolatedLogs`; `tools/run_graphics_ui_contract_tests.ps1`; `tools/run_graphics_settings_persistence_tests.ps1 -NoBuild`; `tools/run_graphics_ui_interaction_smoke.ps1 -NoBuild` | RT signal metrics and scheduler UI pass. Graphics Settings now exposes live RT reflection denoise-alpha and composition-strength sliders, persists them through `RendererTuningState`, packs them into post/denoise shader constants, and reports `ray_tracing.rt_reflection_tuning`; RT showcase passed at `rt_showcase_20260511_001835_557_43944_5990140d`, settings persistence at `graphics_settings_persistence_20260511_001835_386_43292_3db12532`, and UI interaction at `graphics_ui_interaction_20260511_001835_447_39248_9cb0dd9e`. | Reflection scale, roughness thresholds, history clamp/firefly clamp sliders, and a deliberately overbright clamp stress scene remain incomplete. |
...
| P3-3AB | Cinematic Lighting Rigs. | PARTIAL | `RendererLightingRigControl.cpp`, `RendererControlApplier_ScenePresets.cpp`, `src/UI/GraphicsSettingsWindow.cpp`, SRC-SCENES | `tools/run_phase3_visual_matrix.ps1 -NoBuild`; `tools/run_showcase_scene_contract_tests.ps1`; `tools/run_graphics_ui_contract_tests.ps1` | Public scenes report explicit rigs: material_lab_review, sunset_rim, night_emissive, RT gallery. LLM command routing is runtime-tested, and Graphics Settings now exposes Studio/Top Down/Side/Lanterns rig buttons routed through `ApplyLightingRigControl` with the live scene registry. | Native widget click automation for rig selection is still not implemented. |
...
| P3-SLIDER-TONE | Tone/lighting controls: exposure, bloom, contrast, saturation, warmth, sun/god rays/area light/rig. | PARTIAL | SRC-UI-P3, lighting/post state | `tools/run_graphics_ui_contract_tests.ps1`; `tools/run_phase3_visual_matrix.ps1` | Exposure/bloom/god-ray/area-light controls are contract-covered; scene rig presets are runtime-used; Graphics Settings now has explicit rig selection buttons. | Contrast/saturation/warmth sliders and native widget automation remain incomplete. |
...
| P3-SLIDER-RT | RT controls: RT, reflections, GI, reflection scale, roughness, denoise blend, feedback, clamp, firefly, composition strength. | PARTIAL | SRC-RT, SRC-UI-P3 | `tools/run_rt_showcase_smoke.ps1 -NoBuild -IsolatedLogs`; `tools/run_graphics_ui_contract_tests.ps1`; `tools/run_graphics_ui_interaction_smoke.ps1 -NoBuild` | Core toggles/readiness/scheduler visible. Denoise-alpha and composition-strength controls are now implemented as Win32 sliders, persisted, shader-consumed, and frame-contract validated. | Reflection scale, roughness threshold, feedback/history clamp, and firefly clamp controls remain incomplete. |
...
| P3-GATE-10 | RT reflection tuning validation. | PARTIAL | SRC-RT | `tools/run_rt_showcase_smoke.ps1 -NoBuild -IsolatedLogs`; `tools/run_rt_firefly_outlier_scene.ps1 -NoBuild`; `tools/run_graphics_ui_interaction_smoke.ps1 -NoBuild` | RT signal/readiness validated. Firefly/outlier gate passed with raw/history outlier ratios under stricter thresholds. Denoise-alpha and composition-strength controls are now contract- and runtime-smoke validated. | A deliberately overbright/clamped stress scene and clamp-specific controls remain incomplete. |
...
| P3-ORDER-16 | Extend lighting rigs and scene rig selection. | PARTIAL | lighting rig controls, scenes, `src/UI/GraphicsSettingsWindow.cpp` | visual matrix/showcase tests; graphics UI contract; LLM renderer command smoke | Public scenes report explicit rigs; command routing is runtime-tested; Graphics Settings now exposes direct renderer rig buttons. | Native widget automation for the new Graphics Settings rig buttons remains incomplete. |
...
| P3-DOD-07 | Cinematic lighting rigs selectable from scenes, UI, and validation. | PARTIAL | scenes/lighting controls, `src/UI/GraphicsSettingsWindow.cpp` | visual matrix/showcase tests; graphics UI contract; LLM renderer command smoke | Scene/validation selection works; command selection is runtime-tested; Graphics Settings exposes native rig-selection buttons routed through the renderer applier. | Native widget automation for the rig buttons remains incomplete. |
...
| P3-REM-07 | RT reflection tuning sliders and firefly/outlier stress scenes. | PARTIAL | Firefly/outlier gate passes strict RT showcase raw/history outlier thresholds. Denoise-alpha and composition-strength sliders are implemented, shader-consumed, persisted, and frame-contract validated. Reflection scale/roughness/history clamp/firefly clamp controls and a deliberately overbright clamp stress scene remain incomplete. |
...
| P3-REM-12 | Lighting rig selection from UI and commands. | PARTIAL | Scene/validation rigs and LLM command selection now work and are runtime-tested; Graphics Settings source now includes native Studio/Top Down/Side/Lanterns rig controls routed through the live scene registry; native widget automation remains incomplete. |
