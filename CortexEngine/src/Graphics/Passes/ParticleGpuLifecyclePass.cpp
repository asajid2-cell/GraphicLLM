...
DispatchResult Dispatch(const DispatchContext& context) {
    ...
    if (!context.device ||
        ...
        !context.resources ||
        !context.emitters ||
        context.emitterCount == 0 ||
        context.particleCount == 0 ||
        !context.constants ||
        !context.resources->gpuEmitterBuffer ||
        !context.resources->gpuInstanceBuffer ||
        !context.resources->gpuLifecycleConstantsInitialized) {
        return result;
    }

    UINT emitterBytes = 0;
    const HRESULT emitterHr =
        context.resources->UploadGpuEmitters(context.emitters, context.emitterCount, emitterBytes);
    if (FAILED(emitterHr)) {
        return result;
    }

    const D3D12_GPU_VIRTUAL_ADDRESS constantsAddress =
        context.resources->WriteGpuLifecycleConstants(*context.constants);
    if (constantsAddress == 0) {
        return result;
    }

    const HRESULT descriptorHr =
        context.resources->EnsureGpuLifecycleDescriptors(context.device, context.descriptorManager);
    if (FAILED(descriptorHr) ||
        !context.resources->gpuEmitterSrv.IsValid() ||
    ...
    context.commandList->SetComputeRootDescriptorTable(1, context.resources->gpuEmitterSrv.gpu);
    context.commandList->SetComputeRootDescriptorTable(2, context.resources->gpuInstanceUav.gpu);

    const UINT dispatchGroups = (context.particleCount + 127u) / 128u;
    ...
    result.uploadBytes = emitterBytes;
    return result;
}
