...
private:
    ...
    // Allocate transient resources (placed resources) and inject automatic
    // aliasing barriers based on computed lifetimes.
    Result<void> AllocateTransientResources();
    ...
    // Backing heap for transient placed resources (enables true aliasing).
    ComPtr<ID3D12Heap> m_transientHeap;
    uint64_t m_transientHeapSize = 0;
