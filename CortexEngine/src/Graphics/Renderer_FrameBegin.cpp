...
void Renderer::BeginFrame() {
    ...
    if (m_ssao.State().resources.texture) {
        D3D12_RESOURCE_DESC ssaoDesc = m_ssao.State().resources.texture->GetDesc();
    ...
    if (needSSAOResize && m_ssao.State().resources.texture) {
        ...
        if (ssaoResult.IsErr()) {
            ...
            m_ssao.State().controls.enabled = false;
        }
    }
