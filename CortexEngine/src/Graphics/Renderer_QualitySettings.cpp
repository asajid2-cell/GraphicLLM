...
void Renderer::SetBloomIntensity(float intensity) {
    ...
    if (std::abs(clamped - m_bloom.State().controls.intensity) < 1e-6f) {
        return;
    }
    m_bloom.State().controls.intensity = clamped;
    spdlog::info("Renderer bloom intensity set to {}", m_bloom.State().controls.intensity);
}
...
void Renderer::SetBloomShape(float threshold, float softKnee, float maxContribution) {
    ...
    if (std::abs(clampedThreshold - m_bloom.State().controls.threshold) < 1e-4f &&
        std::abs(clampedSoftKnee - m_bloom.State().controls.softKnee) < 1e-4f &&
        std::abs(clampedMaxContribution - m_bloom.State().controls.maxContribution) < 1e-4f) {
        return;
    }
    m_bloom.State().controls.threshold = clampedThreshold;
    m_bloom.State().controls.softKnee = clampedSoftKnee;
    m_bloom.State().controls.maxContribution = clampedMaxContribution;
...
void Renderer::ApplySafeQualityPreset() {
    ...
    m_shadows.Resources().controls.mapSize = std::min(m_shadows.Resources().controls.mapSize, 1024.0f);
    for (uint32_t i = 0; i < kShadowCascadeCount; ++i) {
        m_shadows.Resources().controls.cascadeResolutionScale[i] =
            std::min(m_shadows.Resources().controls.cascadeResolutionScale[i], 0.60f);
    }
