...
Renderer::ExecuteBloomInRenderGraph() {
    ...
    for (uint32_t level = 0; level < kBloomLevels; ++level) {
        bloomATemplates[level] = m_bloom.State().resources.texA[level].Get();
        bloomBTemplates[level] = m_bloom.State().resources.texB[level].Get();
    }
    ...
    for (uint32_t level = 0; level < m_bloom.State().resources.activeLevels; ++level) {
        ...
        if (m_bloom.State().resources.texA[level] && keepAForPostProcess) {
            bloomA[level] = m_services.renderGraph->ImportResource(
                m_bloom.State().resources.texA[level].Get(), m_bloom.State().resources.resourceState[level][0], ("BloomA" + std::to_string(level)).c_str());
        }
        if (!useTransientBloom && m_bloom.State().resources.texB[level]) {
            bloomB[level] = m_services.renderGraph->ImportResource(
                m_bloom.State().resources.texB[level].Get(), m_bloom.State().resources.resourceState[level][1], ("BloomB" + std::to_string(level)).c_str());
        }
    }
    ...
    bloomContext.targetRtv = m_bloom.State().resources.graphRtv;
    ...
    bloomContext.fullscreen.srvTable = m_bloom.State().descriptors.srvTables[m_frameRuntime.frameIndex % kFrameCount].data();
    bloomContext.fullscreen.srvTableCount = kBloomDescriptorSlots;
    bloomContext.fullscreen.srvTableValid = m_bloom.State().descriptors.srvTableValid;
    ...
    if (execResult.IsErr()) {
        ...
        m_bloom.State().resources.resourceState[baseLevel][0] = combinedState;
