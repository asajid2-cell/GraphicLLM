...
Renderer::ExecuteBloomInRenderGraph() {
    ...
    for (uint32_t level = 0; level < kBloomLevels; ++level) {
        ...
        savedBloomStates[level][0] = m_bloomResources.resourceState[level][0];
        savedBloomStates[level][1] = m_bloomResources.resourceState[level][1];
        savedBloomRTV[level][0] = m_bloomResources.rtv[level][0];
        savedBloomRTV[level][1] = m_bloomResources.rtv[level][1];
        savedBloomSRV[level][0] = m_bloomResources.srv[level][0];
        savedBloomSRV[level][1] = m_bloomResources.srv[level][1];
    }
    ...
    if (useTransientBloom) {
        for (uint32_t level = 0; level < m_bloomResources.activeLevels; ++level) {
            for (uint32_t ping = 0; ping < 2u; ++ping) {
                ...
                if (!m_bloomResources.graphRtv[level][ping].IsValid()) {
                    ...
                    m_bloomResources.graphRtv[level][ping] = rtvResult.Value();
                }
                if (!m_bloomResources.graphSrv[level][ping].IsValid()) {
                    ...
                    m_bloomResources.graphSrv[level][ping] = srvResult.Value();
                }
                m_bloomResources.rtv[level][ping] = m_bloomResources.graphRtv[level][ping];
                m_bloomResources.srv[level][ping] = m_bloomResources.graphSrv[level][ping];
            }
        }
    }
    ...
    for (uint32_t level = 0; level < m_bloomResources.activeLevels; ++level) {
        ...
        if (m_bloomResources.texA[level] && keepAForPostProcess) {
            bloomA[level] = m_services.renderGraph->ImportResource(
                m_bloomResources.texA[level].Get(), m_bloomResources.resourceState[level][0], ("BloomA" + std::to_string(level)).c_str());
        }
        if (!useTransientBloom && m_bloomResources.texB[level]) {
            bloomB[level] = m_services.renderGraph->ImportResource(
                m_bloomResources.texB[level].Get(), m_bloomResources.resourceState[level][1], ("BloomB" + std::to_string(level)).c_str());
        }
    }
    ...
    auto rewriteBloomViews = [&](ID3D12Resource* resource, uint32_t level, uint32_t ping) -> bool {
        ...
        m_services.device->GetDevice()->CreateRenderTargetView(resource, &rtvDesc, m_bloomResources.rtv[level][ping].cpu);
        ...
        m_services.device->GetDevice()->CreateShaderResourceView(resource, &srvDesc, m_bloomResources.srv[level][ping].cpu);

        if (ping == 0) {
            m_bloomResources.texA[level] = resource;
        } else {
            m_bloomResources.texB[level] = resource;
        }
        return true;
    };
    ...
    auto restoreBloomResources = [&]() {
        for (uint32_t level = 0; level < kBloomLevels; ++level) {
            m_bloomResources.texA[level] = savedBloomA[level];
            m_bloomResources.texB[level] = savedBloomB[level];
            m_bloomResources.resourceState[level][0] = savedBloomStates[level][0];
            m_bloomResources.resourceState[level][1] = savedBloomStates[level][1];
            m_bloomResources.rtv[level][0] = savedBloomRTV[level][0];
            m_bloomResources.rtv[level][1] = savedBloomRTV[level][1];
            m_bloomResources.srv[level][0] = savedBloomSRV[level][0];
            m_bloomResources.srv[level][1] = savedBloomSRV[level][1];
    ...
    bloomContext.renderDownsampleLevel = [&](uint32_t level, const RenderGraph& graph) {
        ...
        m_bloomResources.resourceState[level - 1][0] = kBloomGraphShaderResourceState;
    ...
    bloomContext.renderBlurHorizontal = [&](uint32_t level, const RenderGraph& graph) {
        ...
        m_bloomResources.resourceState[level][0] = kBloomGraphShaderResourceState;
    ...
    bloomContext.renderBlurVertical = [&](uint32_t level, const RenderGraph& graph) {
        ...
        m_bloomResources.resourceState[level][1] = kBloomGraphShaderResourceState;
        m_bloomResources.resourceState[level][0] = D3D12_RESOURCE_STATE_RENDER_TARGET;
        ScopedRenderPassValue<bool> skipTransitions(m_frameDiagnostics.renderGraph.transitions.bloomSkipTransitions, true);
    ...
    if (execResult.IsErr()) {
        ...
        m_bloomResources.resourceState[baseLevel][0] = combinedState;
        result.executed = true;
    }
