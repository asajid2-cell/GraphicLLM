...
Renderer::ExecuteTAAInRenderGraph() {
    ...
    if (!m_temporal.AAState().enabled || !m_pipelineState.taa || !m_temporal.ScreenState().taaIntermediate || !m_services.window) {
        result.fallbackUsed = true;
        result.fallbackReason = "render_graph_taa_feature_disabled_or_resources_missing";
        return result;
    }

    if (!m_temporal.ScreenState().historyColor || !m_temporal.ScreenState().historySRV.IsValid()) {
        result.fallbackUsed = true;
        result.fallbackReason = "render_graph_taa_history_missing";
        return result;
    }
    if (!m_temporal.ScreenState().taaResolveSrvTableValid) {
        result.fallbackUsed = true;
    ...
    const RGResourceHandle historyHandle =
        m_services.renderGraph->ImportResource(m_temporal.ScreenState().historyColor.Get(), m_temporal.ScreenState().historyState, "TAA_History");
    const RGResourceHandle taaIntermediateHandle =
        m_services.renderGraph->ImportResource(m_temporal.ScreenState().taaIntermediate.Get(), m_temporal.ScreenState().taaIntermediateState, "TAA_Intermediate");

    RGResourceHandle velocityHandle{};
    if (m_temporal.ScreenState().velocityBuffer) {
        velocityHandle = m_services.renderGraph->ImportResource(m_temporal.ScreenState().velocityBuffer.Get(), m_temporal.ScreenState().velocityState, "Velocity_TAA");
    }
    ...
    taaContext.seedHistory.historyColor = {m_temporal.ScreenState().historyColor.Get(), &m_temporal.ScreenState().historyState};
    ...
    taaContext.resolveInputs.taaIntermediate = {m_temporal.ScreenState().taaIntermediate.Get(), &m_temporal.ScreenState().taaIntermediateState};
    ...
    taaContext.resolveInputs.velocity = {m_temporal.ScreenState().velocityBuffer.Get(), &m_temporal.ScreenState().velocityState};
    taaContext.resolveInputs.historyColor = {m_temporal.ScreenState().historyColor.Get(), &m_temporal.ScreenState().historyState};
    ...
    auto& resolveTable = m_temporal.ScreenState().taaResolveSrvTables[m_frameRuntime.frameIndex % kFrameCount];
    ...
    taaContext.resolveDescriptors.history = m_temporal.ScreenState().historyColor.Get();
    ...
    taaContext.resolveDescriptors.velocity = m_temporal.ScreenState().velocityBuffer.Get();
    ...
    taaContext.resolve.targetRtv = m_temporal.ScreenState().taaIntermediateRTV;
    ...
    taaContext.copyToHDR.taaIntermediate = {m_temporal.ScreenState().taaIntermediate.Get(), &m_temporal.ScreenState().taaIntermediateState};
    ...
    taaContext.copyToHistory.historyColor = {m_temporal.ScreenState().historyColor.Get(), &m_temporal.ScreenState().historyState};
    taaContext.copyToHistory.taaIntermediate = {m_temporal.ScreenState().taaIntermediate.Get(), &m_temporal.ScreenState().taaIntermediateState};
    ...
    if (execResult.IsErr()) {
        ...
        m_temporal.ScreenState().historyState = m_services.renderGraph->GetResourceState(historyHandle);
        m_temporal.ScreenState().taaIntermediateState = m_services.renderGraph->GetResourceState(taaIntermediateHandle);
