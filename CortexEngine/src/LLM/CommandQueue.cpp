...
void CommandQueue::ExecuteModifyTransform(ModifyTransformCommand* cmd, Scene::ECS_Registry* registry) {
    ...
    // Optional simple orbit helper (center-driven rotation + child parenting).
    bool appliedOrbit = false;
    if (cmd->setOrbit) {
        std::string centerHint;
        const std::string centerName =
            cmd->orbitCenterName.empty() ? cmd->parentName : cmd->orbitCenterName;
        entt::entity center = ResolveTargetWithFocus(centerName, registry, centerHint);
        if (center == entt::null) {
            PushStatus(false, "move/scale failed: orbit center '" + centerName + "' not found (" +
                               (centerHint.empty() ? "no match" : centerHint) + ")");
            spdlog::warn("Orbit center '{}' not found for '{}'", centerName, tagName);
            return;
        }
        if (center == target) {
            PushStatus(false, "move/scale failed: cannot orbit entity around itself");
            spdlog::warn("Refusing to orbit '{}' around itself", tagName);
            return;
        }

        auto& regEntt = registry->GetRegistry();

        // Prevent trivial cycles: do not allow parenting to a descendant.
        entt::entity cur = center;
        while (cur != entt::null) {
            if (cur == target) {
                PushStatus(false, "move/scale failed: orbit would create a cycle");
                spdlog::warn("Refusing to create orbit parenting cycle '{}' -> '{}'", tagName, centerName);
                return;
            }
            if (!regEntt.valid(cur) || !regEntt.all_of<Scene::TransformComponent>(cur)) {
                break;
            }
            const auto& t = regEntt.get<Scene::TransformComponent>(cur);
            cur = t.parent;
        }

        float radius = std::max(cmd->orbitRadius, 0.1f);
        float speed = cmd->orbitSpeed;
        if (!std::isfinite(speed)) {
            speed = 0.6f;
        }
        speed = std::clamp(speed, -10.0f, 10.0f);

        // Attach target as child of center and place at +X at the requested radius.
        transform.parent = center;
        transform.position = glm::vec3(radius, 0.0f, 0.0f);
        transform.rotation = glm::quat(1.0f, 0.0f, 0.0f, 0.0f);

        Scene::RotationComponent* centerRot = nullptr;
        if (regEntt.all_of<Scene::RotationComponent>(center)) {
            centerRot = &regEntt.get<Scene::RotationComponent>(center);
        } else {
            Scene::RotationComponent rot;
            rot.axis = glm::vec3(0.0f, 1.0f, 0.0f);
            rot.speed = speed;
            centerRot = &regEntt.emplace<Scene::RotationComponent>(center, rot);
        }

        centerRot->axis = glm::normalize(glm::vec3(0.0f, 1.0f, 0.0f));
        centerRot->speed = speed;

        summary << "orbit(" << centerName << ",r=" << radius << ",w=" << speed << ") ";
        touched = true;
        appliedOrbit = true;
    }

    // Optional parenting controls for "orbit around X" style commands.
    // If an explicit orbit was applied above, we skip generic parent changes.
    if (!appliedOrbit && cmd->clearParent) {
    ...
    if (!appliedOrbit && cmd->setParent) {
        std::string parentHint;
        entt::entity parent = ResolveTargetWithFocus(cmd->parentName, registry, parentHint);
        if (parent == entt::null) {
            PushStatus(false, "move/scale failed: parent '" + cmd->parentName + "' not found (" +
                               (parentHint.empty() ? "no match" : parentHint) + ")");
            spdlog::warn("Parent '{}' not found for '{}'", cmd->parentName, tagName);
            return;
        }
        if (parent == target) {
            PushStatus(false, "move/scale failed: cannot parent entity to itself");
            spdlog::warn("Refusing to parent '{}' to itself", tagName);
            return;
        }
        ...
        // Prevent trivial cycles: do not allow parenting to a descendant.
        entt::entity cur = parent;
        while (cur != entt::null) {
            if (cur == target) {
                PushStatus(false, "move/scale failed: parenting would create a cycle");
                spdlog::warn("Refusing to create parenting cycle '{}' -> '{}'", tagName, cmd->parentName);
                return;
            }
            if (!regEntt.valid(cur) || !regEntt.all_of<Scene::TransformComponent>(cur)) {
                break;
        ...
        // Snap into the parent's local frame; the editor gizmo or subsequent
        // commands can position the child at the desired orbital offset.
        transform.position = glm::vec3(0.0f);
        transform.rotation = glm::quat(1.0f, 0.0f, 0.0f, 0.0f);
        summary << "parent=" << cmd->parentName << " ";
        touched = true;
    }

    // Optional continuous spin/orbit behaviour driven by RotationComponent.
    if (cmd->stopSpin) {
        if (registry->HasComponent<Scene::RotationComponent>(target)) {
            registry->RemoveComponent<Scene::RotationComponent>(target);
            spdlog::info("Stopped spin on '{}'", tagName);
            summary << "stop_spin ";
            touched = true;
        }
    }
    if (cmd->setSpin) {
        auto& rot = registry->HasComponent<Scene::RotationComponent>(target)
                      ? registry->GetComponent<Scene::RotationComponent>(target)
                      : registry->AddComponent<Scene::RotationComponent>(target);

        glm::vec3 axis = cmd->spinAxis;
        if (!std::isfinite(axis.x) || !std::isfinite(axis.y) || !std::isfinite(axis.z) ||
            glm::length2(axis) < 1e-4f) {
            axis = glm::vec3(0.0f, 1.0f, 0.0f);
        }
        rot.axis = glm::normalize(axis);

        float speed = cmd->spinSpeed;
        if (!std::isfinite(speed)) {
            speed = 1.0f;
        }
        // Clamp spin speed to a safe range in radians/sec to avoid
        // wild motion when the LLM chooses a large value.
        speed = std::clamp(speed, -10.0f, 10.0f);
        rot.speed = speed;

        spdlog::info("Set spin on '{}' axis ({:.2f},{:.2f},{:.2f}) speed {:.2f} rad/s",
                     tagName, rot.axis.x, rot.axis.y, rot.axis.z, rot.speed);
        summary << "spin(" << rot.axis.x << "," << rot.axis.y << "," << rot.axis.z
                << ";w=" << rot.speed << ") ";
        touched = true;
    }
