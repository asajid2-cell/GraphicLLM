...
public:
    ...
    static std::string GetSystemPrompt(bool hasShowcase) {
        ...
        ss << "Allowed commands:\n"
           ...
           << "- modify_transform: target name, position[3], rotation[3] (Euler angles in degrees), scale[3]. You may also include \"mode\":\"relative\" to treat position as an offset and scale as a multiplier, a \"spin\" object like {\"axis\":[0,1,0],\"speed\":1.0} to start or adjust continuous spinning around that axis (radians per second), and/or \"stop_spin\":true to remove any existing spin from the target. You can attach one object to another with \"parent\":\"Name\" (so it moves/orbits with that parent) and detach it again with \"clear_parent\":true.\n"
