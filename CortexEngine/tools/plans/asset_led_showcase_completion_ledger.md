...
- Evidence:
  ...
  - Coastal channel-scale iteration shortened/lowered the lava surface, channel walls, furnace uprights/crossbeam, and rail runs/posts, then raised/widened the hero camera to reduce near-field glowing channel and rail dominance.
  - Release build passed with `cmake --build CortexEngine\build --config Release --target CortexEngine`; `run_scene_seed_contract_tests.ps1`, `run_showcase_scene_contract_tests.ps1`, `run_scene_composition_stability_tests.ps1 -SceneId coastal_cliff_foundry`, and `run_asset_led_scene_contract_tests.ps1 -SceneId coastal_cliff_foundry -RuntimeSmoke -SmokeFrames 45` passed after the channel-scale iteration.
  - High asset-led capture passed: `powershell -NoProfile -ExecutionPolicy Bypass -File CortexEngine/tools/run_public_capture_gallery.ps1 -NoBuild -Quality High -AssetLedOnly -OutputDir CortexEngine/build/bin/logs/coastal_channel_scale_review2 -Width 1920 -Height 1080 -SmokeFrames 90` reported `captures=15 size=1920x1080 preset=public_high`.
  - Focused hero report `CortexEngine/build/bin/logs/runs/public_capture_gallery_20260514_015357_916_21488_502f1f12/coastal_cliff_foundry/frame_report_last.json` recorded `gpu_frame_ms=6.253`, `avg_luma=100.531`, `rt_reflection_signal_avg_luma=0.07698`, `rt_reflection_history_signal_avg_luma=0.07730`, and texture uploads `submitted=9 completed=9 failed=0 pending=0 uploaded=120.00MB`.
  - Manual review rejected `coastal_channel_scale_review2` for public media: foreground rail/channel dominance is reduced, but backdrop slabs, floating-looking rock supports, and primitive channel/platform geometry remain release blockers.
- Remaining work:
...
- Evidence:
  ...
  - Rain tabletop-vignette reframe synchronized the runtime default, hero, glass-closeup, and puddle/chrome cameras around the table/chrome/glass cluster instead of the unsafe drain macro.
  - Release build, scene seed, showcase-scene, composition stability, and rain asset-led runtime contracts passed after the reframe.
  - Asset-led public capture `CortexEngine/build/bin/logs/rain_tabletop_vignette_review` passed with `captures=15 size=1920x1080 preset=public_high`; the rain hero report recorded `gpu_frame_ms=132.458`, `avg_luma=34.903`, `rt_reflection_signal_avg_luma=0.03780`, `rt_reflection_history_signal_avg_luma=0.03888`, and texture uploads `submitted=13 completed=13 failed=0 pending=0 uploaded=66.67MB`.
  - Manual review rejected `rain_tabletop_vignette_review` for final public media: the drain macro failure is removed and the tabletop material vignette is more deliberate, but primitive rear panels/slats and table/block geometry still keep Rain WIP.
  - Rain tabletop-detail iteration lowered/shrank the rear wood/slat blockers, tightened the hero/glass/puddle bookmarks around the tabletop, and added smaller glass/chrome/ceramic table accents after rejecting an oversized first attempt.
  - Release build passed with `cmake --build CortexEngine\build --config Release --target CortexEngine`; `run_scene_seed_contract_tests.ps1`, `run_showcase_scene_contract_tests.ps1`, `run_scene_composition_stability_tests.ps1 -SceneId rain_glass_pavilion`, and `run_asset_led_scene_contract_tests.ps1 -SceneId rain_glass_pavilion -RuntimeSmoke -SmokeFrames 45` passed after the tabletop-detail iteration.
  - High asset-led capture passed: `powershell -NoProfile -ExecutionPolicy Bypass -File CortexEngine/tools/run_public_capture_gallery.ps1 -NoBuild -Quality High -AssetLedOnly -OutputDir CortexEngine/build/bin/logs/rain_tabletop_detail_review2 -Width 1920 -Height 1080 -SmokeFrames 90` reported `captures=15 size=1920x1080 preset=public_high`.
  - Focused hero report `CortexEngine/build/bin/logs/runs/public_capture_gallery_20260514_021728_170_17612_a1ce27ec/rain_glass_pavilion/frame_report_last.json` recorded `gpu_frame_ms=11.069`, `avg_luma=43.736`, `rt_reflection_signal_avg_luma=0.03131`, `rt_reflection_history_signal_avg_luma=0.03180`, and texture uploads `submitted=13 completed=13 failed=0 pending=0 uploaded=66.67MB`.
  - Manual review rejected `rain_tabletop_detail_review2` for public media: tabletop prop scale improved versus the first detail attempt, but the table still reads as blockout geometry and rear panel/studio reflection artifacts remain visible.
- Remaining work:
  - Add high-quality public captures after final art acceptance.
...
- Evidence:
  ...
  - Desert relic scale iteration shrank the bronze ring/pedestal, reduced the ceramic sphere-vessels, lowered high ruin/lintel blocks, reduced arch columns/caps, and tightened the hero camera around the relic.
  - Release build passed with `cmake --build CortexEngine\build --config Release --target CortexEngine`; `run_scene_seed_contract_tests.ps1`, `run_showcase_scene_contract_tests.ps1`, `run_scene_composition_stability_tests.ps1 -SceneId desert_relic_gallery`, and `run_asset_led_scene_contract_tests.ps1 -SceneId desert_relic_gallery -RuntimeSmoke -SmokeFrames 45` passed after the relic scale iteration.
  - High asset-led capture passed: `powershell -NoProfile -ExecutionPolicy Bypass -File CortexEngine/tools/run_public_capture_gallery.ps1 -NoBuild -Quality High -AssetLedOnly -OutputDir CortexEngine/build/bin/logs/desert_relic_scale_review -Width 1920 -Height 1080 -SmokeFrames 90` reported `captures=15 size=1920x1080 preset=public_high`.
  - Focused hero report `CortexEngine/build/bin/logs/runs/public_capture_gallery_20260514_022535_664_39128_84144666/desert_relic_gallery/frame_report_last.json` recorded `gpu_frame_ms=7.134`, `avg_luma=124.914`, `rt_reflection_signal_avg_luma=0.06056`, `rt_reflection_history_signal_avg_luma=0.06056`, and texture uploads `submitted=8 completed=8 failed=0 pending=0 uploaded=77.33MB`.
  - Manual review rejected `desert_relic_scale_review` for public media: the giant reflective vessel failure is reduced and the ring focal read is cleaner, but primitive plinth, rail, column, and backdrop blockout geometry remain release blockers.
- Remaining work:
  - Add high-quality public captures after final art acceptance.
...
- Evidence:
  ...
  - Neon storefront-crop iteration synchronized the runtime/default hero with a moderate right-storefront crop and tightened the materials bookmark around the display case while restoring the particles bookmark to avoid a right-side studio/background artifact.
  - Release build, scene seed, showcase-scene, composition stability, and neon asset-led runtime contracts passed after the crop iteration.
  - Asset-led public capture `CortexEngine/build/bin/logs/neon_storefront_crop_review3` passed with `captures=15 size=1920x1080 preset=public_high`; the neon hero report recorded `gpu_frame_ms=8.095`, `avg_luma=46.505`, `rt_reflection_signal_avg_luma=0.02678`, `rt_reflection_history_signal_avg_luma=0.02680`, and texture uploads `submitted=8 completed=8 failed=0 pending=0 uploaded=77.33MB`.
  - Manual review rejected `neon_storefront_crop_review3` for final public media: the empty alley/backdrop exposure is reduced and the right display details stay visible, but the display case, storefront wall, and signage still read as primitive blockout geometry.
  - Neon storefront-detail iteration added right facade trim/breakup, smaller display merchandise blocks, and additional product detail inside the glass cases after rejecting an oversized first attempt.
  - Release build passed with `cmake --build CortexEngine\build --config Release --target CortexEngine`; `run_scene_seed_contract_tests.ps1`, `run_showcase_scene_contract_tests.ps1`, `run_scene_composition_stability_tests.ps1 -SceneId neon_alley_material_market`, and `run_asset_led_scene_contract_tests.ps1 -SceneId neon_alley_material_market -RuntimeSmoke -SmokeFrames 45` passed after the storefront-detail iteration.
  - High asset-led capture passed: `powershell -NoProfile -ExecutionPolicy Bypass -File CortexEngine/tools/run_public_capture_gallery.ps1 -NoBuild -Quality High -AssetLedOnly -OutputDir CortexEngine/build/bin/logs/neon_storefront_detail_review2 -Width 1920 -Height 1080 -SmokeFrames 90` reported `captures=15 size=1920x1080 preset=public_high`.
  - Focused hero report `CortexEngine/build/bin/logs/runs/public_capture_gallery_20260514_023813_655_39268_558ca394/neon_alley_material_market/frame_report_last.json` recorded `gpu_frame_ms=8.311`, `avg_luma=46.630`, `rt_reflection_signal_avg_luma=0.02716`, `rt_reflection_history_signal_avg_luma=0.02718`, and texture uploads `submitted=8 completed=8 failed=0 pending=0 uploaded=77.33MB`.
  - Manual review rejected `neon_storefront_detail_review2` for public media: the storefront/detail density improved and the first oversized-prop regression is reduced, but the wall, sign, display case, and product blocks still read as box-built primitives.
- Remaining work:
  - Add high-quality public captures after final art acceptance.
...
- Evidence:
  ...
  - Forest narrow-creek iteration tightened the hero camera around the creek/water material read and reduced the creek sheet plus foam strips so the frame depends less on the weak rear blockout.
  - Release build, scene seed, showcase-scene, composition stability, and forest asset-led runtime contracts passed after the narrow-creek iteration.
  - Asset-led public capture `CortexEngine/build/bin/logs/forest_creek_narrow_review` passed with `captures=15 size=1920x1080 preset=public_high`; the forest hero report recorded `gpu_frame_ms=11.058`, `avg_luma=73.618`, `rt_reflection_signal_avg_luma=0.08920`, `rt_reflection_history_signal_avg_luma=0.08915`, and texture uploads `submitted=26 completed=26 failed=0 pending=0 uploaded=173.33MB`.
  - Manual review rejected `forest_creek_narrow_review` for public media: the tighter crop improves water focus, but flat green platforms, hard water strips, a block-built shrine, floating-looking vegetation/rocks, and sky-wall dependency remain release blockers.
  - Forest creek-edge iteration narrowed the creek sheet, shortened hard foam/edge strips, enlarged creek step rocks, and reduced foreground hero ferns after rejecting the first pass for making the strip tokens dominant.
  - Release build passed with `cmake --build CortexEngine\build --config Release --target CortexEngine`; `run_scene_seed_contract_tests.ps1`, `run_showcase_scene_contract_tests.ps1`, `run_scene_composition_stability_tests.ps1 -SceneId forest_creek_shrine`, and `run_asset_led_scene_contract_tests.ps1 -SceneId forest_creek_shrine -RuntimeSmoke -SmokeFrames 45` passed after the creek-edge iteration.
  - High asset-led capture passed: `powershell -NoProfile -ExecutionPolicy Bypass -File CortexEngine/tools/run_public_capture_gallery.ps1 -NoBuild -Quality High -AssetLedOnly -OutputDir CortexEngine/build/bin/logs/forest_creek_edge_review2 -Width 1920 -Height 1080 -SmokeFrames 90` reported `captures=15 size=1920x1080 preset=public_high`.
  - Focused hero report `CortexEngine/build/bin/logs/runs/public_capture_gallery_20260514_025230_968_51416_c68ac191/forest_creek_shrine/frame_report_last.json` recorded `gpu_frame_ms=10.638`, `avg_luma=83.785`, `rt_reflection_signal_avg_luma=0.10987`, `rt_reflection_history_signal_avg_luma=0.10984`, and texture uploads `submitted=26 completed=26 failed=0 pending=0 uploaded=173.33MB`.
  - Manual review rejected `forest_creek_edge_review2` for public media: the oversized foreground fern failure is reduced and the strip tokens are shorter, but the water plane, green strip artifacts, block-built shrine, and sky/backdrop dependency remain release blockers.
- Remaining work:
  - Add high-quality public captures after final art acceptance.
...
- Evidence:
  ...
  - Manual review recorded asset-led public capture `rain_tabletop_vignette_review`; the drain macro failure is removed and the table/chrome/glass crop improves, but primitive rear panels/slats and block geometry keep Rain WIP and unsuitable for publication.
  - Manual review recorded asset-led public capture `rain_tabletop_detail_review2`; smaller tabletop accents and a slightly wider crop improve the oversized-prop regression, but blockout table geometry and rear panel/studio reflection artifacts keep Rain WIP and unsuitable for publication.
  ...
  - Manual review recorded asset-led public capture `neon_storefront_crop_review3`; the right-storefront crop reduces empty alley exposure, but primitive wall/case/sign geometry keeps Neon WIP and unsuitable for publication.
  - Manual review recorded asset-led public capture `neon_storefront_detail_review2`; extra facade trim and smaller display merchandise improve density, but the wall/sign/case/product geometry remains visibly box-built and unsuitable for publication.
  ...
  - Manual review recorded asset-led public capture `coastal_channel_scale_review2`; improved scale/framing reduces the near-field rail/channel read, but backdrop slabs, floating-looking rock supports, and primitive platform geometry keep Coastal WIP and unsuitable for publication.
  ...
  - Manual review recorded asset-led public capture `desert_relic_scale_review`; smaller vessel/column massing improves the relic read, but primitive plinth, rail, column, and backdrop geometry keep Desert WIP and unsuitable for publication.
  - Manual review recorded asset-led public capture `forest_creek_narrow_review`; the tighter creek crop improves water focus, but flat platforms, hard strip edges, a block-built shrine, and sky-wall dependency keep Forest WIP and unsuitable for publication.
  - Manual review recorded asset-led public capture `forest_creek_edge_review2`; reduced foreground fern scale and shorter edge strips improve the crop, but the water plane, strip artifacts, block shrine, and sky/backdrop dependency keep Forest WIP and unsuitable for publication.
