...
try {
    ...
    $manifestPath = Join-Path $outputPath "manifest.json"
    & python $v3SceneProfileAnalyzer --manifest $manifestPath --output-json $sceneProfileOutput --output-md $sceneProfileMarkdown --min-family-count 1
    if ($LASTEXITCODE -ne 0) {
        exit $LASTEXITCODE
    }
