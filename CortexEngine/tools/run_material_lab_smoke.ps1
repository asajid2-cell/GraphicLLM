param(
    ...
    [double]$MaxVisualAvgLuma = 220.0,
    [double]$MinVisualCenterLuma = 110.0,
    [double]$MaxVisualCenterLuma = 220.0,
...
if (-not [string]::IsNullOrWhiteSpace($LogDir)) {
    ...
    $runId = "material_lab_{0}_{1}_{2}" -f `
...
if ($exitCode -ne 0) {
    Add-Failure "Material Lab smoke process failed with exit code $exitCode"
}
...
if ($failures.Count -eq 0) {
    ...
    if ([string]$report.frame_contract.environment.active -ne "cool_overcast") {
        Add-Failure "environment is '$($report.frame_contract.environment.active)', expected cool_overcast"
    }
    if ([string]$report.frame_contract.environment.budget_class -ne "medium") {
        Add-Failure "environment budget class is '$($report.frame_contract.environment.budget_class)', expected medium"
    }
    if ([int]$report.frame_contract.environment.max_runtime_dimension -lt 4096) {
        Add-Failure "environment max_runtime_dimension is $($report.frame_contract.environment.max_runtime_dimension), expected >= 4096"
    }
    ...
    if ([string]$report.frame_contract.lighting.rig_id -ne "material_lab_review") {
        Add-Failure "lighting rig is '$($report.frame_contract.lighting.rig_id)', expected material_lab_review"
    }
    ...
    if ([int]$materials.surface_mirror -lt 1 -or
        [int]$materials.surface_glass -lt 1 -or
        [int]$materials.surface_plastic -lt 1 -or
        [int]$materials.surface_masonry -lt 1 -or
        [int]$materials.surface_emissive -lt 1 -or
...
if ($failures.Count -gt 0) {
    Write-Host "Material Lab smoke failed:" -ForegroundColor Red
...
Write-Host "Material Lab smoke passed" -ForegroundColor Green
